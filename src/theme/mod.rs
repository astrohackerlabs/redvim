...
fn compatible_scopes(scope: &str) -> Vec<String> {
    ...
    // Neovim queries use these captures where imported TextMate themes use
...
mod tests {
    ...
    #[test]
    fn nu_capture_fallbacks_preserve_explicit_theme_styles() {
        let mut theme = parse_vscode_theme("themes/mocha.json").unwrap();
        assert_eq!(
            theme.get_style("number"),
            theme.get_style("constant.builtin")
        );
        assert_eq!(
            theme.get_style("variable"),
            theme.get_style("variable.other.readwrite")
        );
        let explicit = Style {
        ...
        theme.token_styles.push(TokenStyle {
            name: None,
            scope: vec!["number".into(), "variable".into()],
            style: explicit.clone(),
        });
        assert_eq!(theme.get_style("number.float"), Some(explicit.clone()));
        assert_eq!(theme.get_style("variable"), Some(explicit));
    }
