...
fn language_definitions() -> Vec<BundledLanguageDefinition> {
    vec![
        BundledLanguageDefinition {
            ...
            extensions: &["nu"],
            filenames: &[],
            language: Some(|| tree_sitter_nu::LANGUAGE.into()),
...
# -let ready = false;\n";
    ...
    #[test]
    fn nu_bundled_parser_queries_and_theme_highlight_real_syntax() {
        let code = include_str!("queries/highlights/nu-smoke.nu");
        let language = tree_sitter_nu::LANGUAGE.into();
        let mut parser = Parser::new();
        parser.set_language(&language).unwrap();
        let tree = parser.parse(code, None).unwrap();
        assert!(
        ...
        let mut highlighter = Highlighter::new(&theme).unwrap();
        let styles = highlighter
            .highlight_for_file(Some("example.nu"), code)
            .unwrap();
        for (token, scope) in [
            ("# café", "comment"),
            ...
            ("5sec", "number"),
            ("| str", "operator"),
            ("each", "function.builtin"),
            ("loud]", "attribute"),
        ] {
            let expected = theme
                .get_style(scope)
                .unwrap_or_else(|| panic!("missing {scope}"));
            let actual = effective_style_at(&styles, code.find(token).unwrap());
            assert_eq!(actual, Some(&expected), "{token}: {scope}");
            if scope != "variable" {
                assert_ne!(expected.fg, theme.style.fg, "{scope} needs a visible color");
            }
        }
    }

    #[test]
    fn nu_is_bundled_without_external_configuration_or_trust() {
        let directory = tempfile::tempdir().unwrap();
        let registry = LanguageRegistry::from_config(&HashMap::new(), directory.path()).unwrap();
        assert!(matches!(
            registry.languages["nu"].grammar,
            Some(GrammarSource::Bundled(_))
        ));
        assert_eq!(std::fs::read_dir(directory.path()).unwrap().count(), 0);
        let mut highlighter =
            Highlighter::with_registry(&theme_with_scopes(&["keyword"]), Arc::new(registry))
                .unwrap();
        assert_eq!(
            highlighter.language_id_for_file(Some("example.nu")),
            Some("nu")
        );
        assert_eq!(highlighter.language_id_for_name("nushell"), Some("nu"));
        assert_eq!(highlighter.language_id_for_name("nu"), Some("nu"));
        assert_eq!(
            highlighter.language_id_for_source(Some("script"), "#!/usr/bin/env nu\n"),
            Some("nu")
        );
        for alias in ["nu", "nushell"] {
            let code = format!("```{alias}\nlet answer = 42\n```\n");
            let styles = highlighter.highlight("markdown", &code).unwrap();
            assert_token_highlighted(&styles, &code, "let");
        }
    }

    #[test]
    fn nu_highlighting_recovers_from_incomplete_and_changed_tokens() {
        let mut incremental = highlighter();
        for code in [
            "let value = \"hello\"\n",
            "let value = \"",
            ...
            "let value = $\"café ($env.USER)\"\n",
        ] {
            let actual = incremental.highlight("nu", code).unwrap();
            let expected = highlighter().highlight("nu", code).unwrap();
            let shape = |spans: &[StyleInfo]| {
                spans
                    .iter()
                    .map(|s| (s.start, s.end, s.style.clone()))
                    .collect::<Vec<_>>()
            };
            assert_eq!(shape(&actual), shape(&expected), "{code}");
            assert_token_highlighted(&actual, code, code.split_whitespace().next().unwrap());
        }
    }
