...
//!
//! Plugins and themes resolve in the order user configuration, `REDVIM_RUNTIME`, then
...
pub enum RuntimeAssetSource {
    ...
    User,
    /// Development directory selected by `REDVIM_RUNTIME`.
    Runtime,
...
pub fn eject_runtime_asset(asset: &str, config_dir: &Path, force: bool) -> anyhow::Result<PathBuf> {
    ...
    let source = resolve_non_user_runtime_asset(kind, &file, config_dir).ok_or_else(|| {
        anyhow::anyhow!(
            "runtime asset `{}/{}` was not found in $REDVIM_RUNTIME or embedded assets",
            kind.dir_name(),
            file
        )
    })?;
...
mod tests {
    ...
    impl RedRuntimeGuard<'_> {
        fn set(path: &Path) -> Self {
            ...
            let previous = env::var_os("REDVIM_RUNTIME");
            env::set_var("REDVIM_RUNTIME", path);
            Self {
                _guard: guard,
                previous,
            }
    ...
    impl Drop for RedRuntimeGuard<'_> {
        fn drop(&mut self) {
            if let Some(previous) = &self.previous {
                env::set_var("REDVIM_RUNTIME", previous);
            } else {
                env::remove_var("REDVIM_RUNTIME");
            }
        }
    ...
    fn runtime_files_marks_shadowed_sources() {
        ...
        assert!(files.contains("Resolution order: user config, $REDVIM_RUNTIME, embedded."));
        let line = files
            .lines()
