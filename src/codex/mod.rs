...
async fn run<H: CodexToolHost>(
    ...
    match result {
        Ok(()) if exit_status.is_some_and(|status| !status.success()) => {
            ...
            if !stderr.is_empty() {
                message.push_str("; Codex wrote diagnostic details to the RedVim log");
            }
