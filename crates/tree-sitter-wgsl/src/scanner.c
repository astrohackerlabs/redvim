...
// RedVim: consume a nested comment after its opening delimiter. Consume each
// code point once, including adjacent stars/slashes at nested boundaries.
static bool lexer_finish_block_comment(Lexer *lexer) {
  size_t nesting = 1;
  while (nesting > 0 && lexer_peek(lexer) != kEOF) {
    CodePoint current = lexer_next(lexer);
    if (current == '/' && lexer_match(lexer, '*')) {
      nesting++;
    } else if (current == '*' && lexer_match(lexer, '/')) {
      nesting--;
    }
  }
  return nesting == 0;
...
/// Attempts to match a /* block comment */. A failed external scan is rewound
/// by Tree-sitter, so consuming the first slash on failure is safe here.
static bool lexer_match_block_comment(Lexer *lexer) {
  return lexer_match(lexer, '/') && lexer_match(lexer, '*') &&
         lexer_finish_block_comment(lexer);
}
...
// Only for speculative template classification (mark_end precedes lookahead).
// A non-comment slash may be consumed: division does not affect delimiter
// pairing, and /= retains its '=' so the assignment rejection still applies.
static void lexer_skip_template_trivia(Lexer *lexer) {
  for (;;) {
    lexer_skip_blankspace(lexer);
    if (!lexer_match(lexer, '/')) return;
    if (lexer_match(lexer, '/')) {
      while (lexer_peek(lexer) != kEOF && lexer_peek(lexer) != '\n' &&
             lexer_peek(lexer) != '\r') {
        lexer_next(lexer);
      }
    } else if (lexer_match(lexer, '*')) {
      if (!lexer_finish_block_comment(lexer)) return;
    } else {
      return;
    }
  }
...
static void classify_template_args(Scanner *scanner, Lexer *lexer) {
  ...
  while (!stack_entry_array_empty(&lt_stack) && !lexer_match(lexer, kEOF)) {
    lexer_skip_template_trivia(lexer);
